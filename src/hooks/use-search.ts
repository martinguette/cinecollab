...
export function useSearch() {
  ...
  const fetchResults = useCallback(async (resetResults = false) => {
    if (!debouncedQuery) {
    ...
    try {
      ...
      setResults(prev => resetResults ? data.results : [...prev, ...data.results]);
  ...
  }, [debouncedQuery, filters.genres, filters.region, filters.year, page]);
