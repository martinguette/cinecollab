...
export function SearchFiltersPanel({ filters, onFiltersChange, onClose }: SearchFiltersPanelProps) {
  ...
  const hasFilters = filters.genres.length > 0 || !!filters.year || !!filters.region;
      ...
      <div className="space-y-3">
        ...
        <div className="space-y-2">
          ...
          <div className="flex flex-wrap gap-2 max-h-40 overflow-y-auto p-1">
            {loading ? (
              <div className="w-full text-center text-sm text-muted-foreground">Loading genres...</div>
            ) : (
              allGenres.map((genre) => (
                <Badge
                  key={genre.id}
